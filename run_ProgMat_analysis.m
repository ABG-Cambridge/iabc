...
% This script analyses the Progressive Matrices (Raven's matrices) task results collected from the iABC app
...
for tr = 1:length(data.results.items)
    ...
    if isfield(data.results.items{tr},'trial')
        ...
        processed_data.trials(trial_idx).response = data.results.items{tr}.response;
        if isfield(data.results.items{tr},'responseTime')
            processed_data.trials(trial_idx).RT = data.results.items{tr}.responseTime;
            processed_data.trials(trial_idx).correct = data.results.items{tr}.responseCorrect;
        else
...
fprintf('"None of the above" responses = %d\n', sum([processed_data.trials.response]==7));
fprintf('"I don''t know" responses = %d\n', sum([processed_data.trials.response]==8));
